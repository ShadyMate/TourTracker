...
- **Image upload** — Attach an image to any tour via `POST /tours/{id}/image`; images are stored on the server filesystem and only the filename is kept in the database; the old image is cleaned up automatically on replacement or tour deletion
